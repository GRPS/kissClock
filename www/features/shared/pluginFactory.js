...
.factory('DBA', function($state, $cordovaSQLite, $q, $ionicPlatform, config) {
    ...
    self.setup = function() {
        ...
        self.init()
            ...
            .then(function(cnt) {
                if(cnt == 0) {
                    self.query("INSERT INTO Config (key, obj) values (?, ?)", ["config", angular.toJson(options)]);
                }
            })
            .then(function() {
                    return self.query("SELECT c.rowid AS id, c.key AS key, c.obj AS obj FROM Config c WHERE c.rowid = 1");
                })
            .then(function(result){
                record = self.getFirst(result);
                o = JSON.parse(record.obj);
                config.colour = o.colour;
                config.date = o.date;
                config.font = o.font;
                config.time = o.time;
